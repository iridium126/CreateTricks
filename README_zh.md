...
- **加热压实**（Heated Compacting）——为动力辊压机新增的加热盆配方。将紫水晶粉/碎片/充能紫水晶转化为液态媒质（输出量根据配置文件动态重算）。
- **雾化压实**（Mist Compacting）——需要或产生雾副产物的辊压配方。
...
所有配置项均位于服务器配置文件（`createmanaindustry-server.toml`）：
