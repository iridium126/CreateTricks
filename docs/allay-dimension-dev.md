...
| **4i. iris 光影集成（voxy 契约，2026-09-05 grilling 定稿 9 问）** 🔨 **G0+G1 已实现（G0 已冒烟；G1 待进维度验证）** | **范围**：allay 维度地形渲染（V0+GPU 双路径、L0）通过 voxy 契约获得 pack 光照；覆盖 Photon v1.3b + Complementary Unbound r5.8.1（均实测自带 voxy.json）。**决策**：①路线=voxy 契约唯一（flw-compat 式源码合并挂起）；②与 voxy 移植版（.refs/voxy-backport，Fabric 0.2.15-beta）**完美共存**：自有 duck 接口（IGetAllvrPatchData/IGetAllvrPipelineData）与字段名避免 mixin 硬冲突，共享面（VOXY define、vx* uniforms/samplers、colortex16-19 扩展、SourceNames 登记）以 `mixin.allvriris.shared.*` 包门控在 voxy 在场时整体让位（voxy 的 PackRenderTargetDirectives 是 @Redirect 不可链式，故己方用 @WrapOperation 且互斥）；③**运行时 VOXY define 为每维度注入**——iris 1.7.3/1.8.14 的环境 define 是 pack 全局的（sourceProvider 单一 lambda），故注入点选 ShaderPack 内唯一的 glslPreprocessSource 调用（@WrapOperation method="lambda*"）+ Iris.createPipeline 线程LOCAL 维度捕获，allay 之外零暴露；④±30M float 精度接受并文档化（composite 侧无解，patch 侧消费面近零）；⑤TAA jitter 纳入（voxy.json 经 iris sourceProvider 预处理，条件 JSON 免费消解），renderScale/TAAU 缩放不做；⑥lightmap=mesher 期烘焙（天光列扫描+nonEmptyBlockCount 早退、emitter 曼哈顿衰减、quad reserved 8bit）+ **shadow-pass 参与**（深度-only MDI 进 shadow FBO，独立子开关）；⑦customId 存 stateTable spare texel，pack 切换全量重解析零重 mesh；⑧开关 allvrIrisIntegration（默认 false）/allvrIrisShadowPass（默认 true），回退链 pack-lit→unpatched 直写 colortex0→AFTER_LEVEL 自发光。**已知事实修正**：运行时 iris = **1.8.14-beta.1+mc1.21.1**（run/mods），非 .refs/Iris 的 1.7.3 源码；G1 编码按 1.8.14 实际 API（javap 核对：cached uniform 类在 uniforms.custom.cached 包、SamplerHolder 直收 GlSampler、LightTexture.lightTexture 私有→accessor mixin、glBlendFuncSeparate 在 GL14、IdMap 在 shaderpack 包）。**G1 已落地**：AllvrVoxyPatch（解析器，失败不抛出降级回退）、AllvrIrisPipelineData（draw targets/UBO 布局算法/sampler shim/SSBO/blending 求值）、AllvrVoxyUniforms（vx* 供给，待 draw 切片喂数值）、AllvrVoxySamplers、10 个 mixin（4 核心+4 共享+2 accessor+1 LightTexture accessor）、配置两项、VOXY_PORT_ACTIVE 旗标、插件分组门控。**G2-G5 待做**：draw 挂载（FBO+patched fsh+lightmap 烘焙+shadow pass+uniform 每帧上传）→ 冒烟验收 | G1 验收=进维度加载 Photon/Complementary 后日志 `[Allvr] voxy patch resolved` 且 uniforms-not-found 为空、无 mixin 应用冲突；G2=pack-lit 视觉；G3=移植版共存冒烟；G4=无 pack 逐字节回归；G5=性能 | 阶段 4 |
