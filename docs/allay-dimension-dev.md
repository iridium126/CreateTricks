...
标准三轴扫描：对每个轴 × 两个方向，做 32×32 的 2D 面罩动态合并（同 stateId 且同为"暴露面"合并为最大矩形，上限 32——4i 排查⑩曾收紧到 4，⑪改由 fragment-exact shadow pass 在绘制期保证 shadow 对大 quad 的精确性后恢复 32，见 4i 行）。暴露面判定 = 面邻体素 `stateTable.opaque == false`（cutout 类视作非遮挡，与原版语义一致）。输出 8 B/quad 流 + 8 组计数。复杂度 O(32³/轴)，单 cube 微秒级。
...
| R23 | shadow-pass 绘制时机 = AFTER_BLOCK_ENTITIES（deferred 采样 shadowtex 前有效），但 pack 的 shadowcolor 过滤/mip 生成早已跑完（不含 ALLVR 深度）——彩色阴影过滤对 ALLVR 地形不生效；shadowtex1 = shadow pass 内 blit 的 opaque 快照 + 我们的补写（逐帧全量重绘 LEQUAL 覆盖，语义与"opaque-only"一致）；shadow 程序按 pack 约定应用 distortion（复用 client/render/shaderpack 体系），新 pack 需注册新 convention 或退 mode 0 | 低（深度-only 覆盖主流 PCF pack） | 彩色阴影对地形非必需；若 pack 依赖 shadowcolor 过滤的地形彩影再评估提前 hook 点 |
