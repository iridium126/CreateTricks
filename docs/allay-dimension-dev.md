...
> 状态：**阶段 1–2 已实现（2026-09-03）；阶段 2 冒烟测试发现 2 个客户端 bug（无碰撞坠落 / 破坏方块 AIOOBE），已修复（2026-09-04，§2.5）；阶段 3（ALLVR V0）已实现（2026-09-04，待客户端冒烟测试）**。实现偏差与新增技术事实见 §4.2/§4.3/§2.4/§2.5/§5.2/§6.4 与 §13 阶段 1–3 行。
> 目标：
...
4. **服务端 `flags&2`（sendBlockUpdated 等价物）仍为空跳过**：服务端方块变更不主动推送，单机自洽靠客户端预测，多人/重进靠重新订阅全量重发——**增量同步显式后置阶段 3**（dirty cube 重发通路，与渲染共用）。
