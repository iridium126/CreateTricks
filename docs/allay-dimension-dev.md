...
| R23 | shadow-pass 绘制时机 = AFTER_SOLID_BLOCKS（deferred 采样 shadowtex 前有效），但 pack 的 shadowcolor 过滤/mip 生成早已跑完（不含 ALLVR 深度）——彩色阴影过滤对 ALLVR 地形不生效；极端 \|Y\|≈30M 时 world-space 绘制 float32 量化 2 格（R20 同口径） | 低（深度-only 覆盖主流 PCF pack） | 彩色阴影对地形非必需；量化在 \|Y\|≤100 万 <0.1 格不可感知；若 pack 依赖 shadowcolor 过滤的地形彩影再评估提前 hook 点 |
