...
| `client/particles/emitter/EmitterSpec.java` | 不可变发射器规格 + builder；`pack()` 打成 20×vec4 GPU 头（material/collide/flutter/spin/spriteCount/bakeIndex/**animation**/heldItem(MODEL)/lightmap(贴图)） |
| `client/particles/emitter/EmitterShape.java` | `POINT / BOX / SPHERE / CONE` |
| `client/particles/engine/HexSpecs.java` | **Hexcasting 咏唱喷洒规格**（spawnStyle 4 + colorMode 4 post-pack）：ConjureParticle 连续化常量（drag 0.8165/s、上浮 0.16 b/s²、寿命 (3.2, 4.27]s、尺寸 0.135×[2/3,4/3]）+ 颜料轮采样（真实 `FrozenPigment.getColor` 沿梯度轴 8 方向烘焙，含最低亮度）/cmip 预设（紫水晶/UUID/彩虹） |
...
| `additive.vsh` | `gl_InstanceID` 经 orderAdd 排列（keygen 视锥剔除后的可见加法粒子集）从粒子 SSBO 取数、相机朝向 billboard、尺寸/颜色/透明度关键帧、每发射器 glow；**colorMode 4（Hexcasting 颜料轮）**：8 关键帧槽=色轮，相位=速度方向在固定梯度轴上的投影（三次缓动 morph，逐喷洒时间相位烘焙进采样），精确指数缩小 `e^(−0.8165·age)`（= 0.96/tick），flat varying 把六边形标志传给 fsh |
| `additive.fsh` | 软圆衰减 + 距离淡出 + 叠加输出；vHex=1 时切换**程序化六边形柔光斑**（复刻 hexcasting cloud.png：尖顶六边形 SDF、外接半径 0.453 UV、内部平台 1.0、~2.5 纹素软边） |
...
`origin(占位)` / `shape,speed,radius` / `gravity,drag` / `accel,windStrength` / `windDir,rotation` / `life,sizeStart,sizeEnd` / `sizeEase,coneTanHalf,colorCount,glow` / `material(0 ADD/1 ALPHA/2 MODEL/3 OPAQUE),collideMode,flutter,spin` / `8×RGBA 颜色关键帧` / `bakeIndex,spriteCount,0,0` / `animation(0 FLY..3 DEATH),spawnStyle(风暴后写),heldItem(MODEL)/lightMode(贴图——17.z 按材质分语义),0` / 保留
...
### 碰撞纹理（GL_TEXTURE_3D R8）：`48 × 32 × (48×K)`，K=8 个 3D 切片堆叠；bakeMeta（每槽 origin.xyz + presence）。**烘焙谓词 = 碰撞外形非空**（`getCollisionShape(chunk,pos)` 2 参重载走每状态缓存形状 `Cache.collisionShape`——状态初始化时以 EmptyBlockGetter 预计算、零 level 访问，离线程与 `getBlockState` 同级良性竞争容忍）：碰撞外形为空的方块（草/花/火把/作物/铁轨/流体）不烘焙，粒子直接穿过（原版粒子对齐）；台阶/雪片仍整格烘焙（二元体素限制）。**风暴象限中心化**（ensureQuadrants）：接缝过锚点，2×2 覆盖恒为锚点 ±48 XZ（不随世界格纹摆动，消除最坏边 24 格；Y 带恒 ±16）。**MODEL 碰撞箱对齐**（原版 Allay `sized(0.35, 0.6)`）：扫掠测试点水平前伸 0.175（半宽——模型表面贴墙而非半身嵌墙）、向上前伸 0.6（测头而非脚——脚点检测永远看不见天花板）、向下不伸（脚点=箱底，落地贴地）；**上移测头路径两端**（`pos+0.6` 与 `pos+sy+0.6`——头部可经水平移动先探入天花板，而 0.5 子步 + 0.6 前伸 > 1 格，单端点探针会整格跳过 1 格厚天花板；两端相距 ≤0.5 < 1 格，几何密闭），精灵因 `marginYUp == 0` 短路不付额外 fetch；前伸是探针线常量偏移，采样间距仍一子步 <1 格无隧穿；仅 MODEL 启用，精灵保持点测试。**卡死逃逸 v2**（update.comp 扫掠前，当前体素实心触发）：① 来向轴优先——沿 −vel 主轴扫 ≤8 体素，找到自由体素即退出并**跳过其余所有面**；② 否则六向各扫 ≤8 取**最近**自由体素；③ 全无开口 → 0.5 格步长限步直列上升兜底（≤4 格/帧，更深下一帧续）。退出**仅清零退出轴速度分量**（原版逐轴碰撞响应，切向运动保留）；逃逸帧独占位置更新（跳过扫掠）；+1 次 texelFetch/帧/粒子为稳态成本，扫描只在真实嵌入时运行；静止粒子不受影响（位置在体素顶面，solidAt 的 +0.0001 epsilon 读作上方空体素）。**solidAt 接缝感知**：界内快路径（1 fetch）不变；越界点回退遍历 bakeMeta 找真实包含该点的切片再测（O(8)，仅发生在接缝 ±1 格内或逃逸扫描）——风暴 2×2 象限接缝过锚点（整数 BlockPos=地面方块缝），越界即自由在接缝处是说谎：逃逸横扫描把嵌入成员"退出"到接缝另一侧的地板方块（+x 距离 1 与真实向上出口打平、按检查次序获胜），逐帧 ±1 格乒乓=闪烁工件；修复后扫描读邻居真值、向上出口获胜，顺带闭合扫掠探针在接缝处穿墙的旧隐患
...
| cherry_leaves | 忠实复刻 `CherryParticle`：300t 寿命/0.3 块/s² 重力/0.05 或 0.075 两档尺寸（emit.comp 按 flutter>0 键控 50/50）/flutter 螺旋/自旋 ±30°/s 起 ±5°/s² 加速（vsh 解析式）/12 帧/触地当帧移除/出生点世界光照（lightmap 标志 → 17.z，战斗粒子同机制） | flutter 换算成块/s²（×1.0 系数）；自旋/选帧在 vsh 由 seed 解析，不占粒子数据；光照冻结在出生点（15 s 一生不重采样）为已知取舍 |
...
33. **cherry 原版对齐二轮（逐项核对 `CherryParticle` 1.21.1 源码）** → ①**自旋速率 20×/400× 过快**：vsh 把原版 `rotSpeed=±30°`/`spinAcceleration=±5°`（角度量）误读为每 tick 的 roll 速率/加速度再 ×20/×400；原版 roll 速率 = `rotSpeed/20`（±1.5°/t = ±30°/s）、roll 加速率 = `spinAcceleration/400`（±5°/s²），解析式改为 `roll = ±30°·t + 2.5°·t²`（300t 总转角 ≈1012°，与逐 tick 累加一致）；②**全亮 → 出生点世界光照**：`Particle.getLightColor` 是真实光图采样，新增 `EmitterSpec.Builder.lightmap` → header 17.z（与 MODEL 的 heldItem 材质分语义同槽），classic 命令的 `emitLight` 槽按标志在渲染线程打包时采样（复用战斗粒子机制），emit.comp classic 写池 p2.w = lightMode ? lightPacked : 1.0（光照 15 s 冻结在出生点为已知取舍）；③**classic 通用路径删三项抖动**（±30% 尺寸、±30% 亮度 intensity、初始 roll ±π）——审计确认战斗粒子组走独立 spawnStyle 分支（其内部 mix(0.7,1.3) 是复刻 `SingleQuadParticle` 默认 quadSize 连续分布的手段，保留）、MODEL 在 shader 内已跳过、受影响者仅 6 个 ADDITIVE 测试特效（无 gameplay 引用）；④cherry 原版 50/50 两档 quadSize 0.05/0.075 回归：emit.comp 按 flutter>0 键控 `baseSize ∈ {1, 2/3}`（唯一 flutter 用户）；⑤**触地当帧移除**：原版 `remove()` 在 tick 末渲染前生效，落地帧不再画——update.comp 落地时直接不写输出（替代旧的"画一帧再消失"）
35. **Hexcasting 粒子移植**（ConjureParticle + ParticleSpray + 颜料 → GPU 引擎）→ ①**spawnStyle 4**（emit.comp）：`MsgCastParticleS2C.Handler` 采样精确复刻——球状 fuzz 位置偏移（半径线性于 u，原版原样）、绕喷洒轴的 acos 均匀锥速度（法向 `v×(0,0,1)` 带轴平行守卫）、固定随机 roll、寿命 `64/((r+3)·0.25)` tick 连续化、尺寸 [0.1,0.2)×0.9；**c 槽复用**（本 style 不用光照/成员/环槽）：`{vel.xyz b/s, intBitsToFloat(fuzz16|spread16<<16)}`——零步长扩容、零头槽位冲突；②**colorMode 4 颜料轮**（additive.vsh）：8 关键帧槽=色轮，相位=速度方向·固定梯度轴 normalize(0.3,0.8,0.5)（与 HexSpecs.GRADIENT_DIR 对齐），三次缓动 morph，逐喷洒时间相位烘焙进采样（原版每粒子在生成瞬间冻结颜色→单次喷洒内方向是唯一活跃维度）；指数缩小精确式 `e^(−0.8165·age)`；③**六边形柔光斑**（additive.fsh）：程序化复刻 cloud.png（尖顶六边形 SDF、纯白 RGB/形状全在 alpha——平台 1.0、软边 2.5 纹素），flat varying 门控、其他加性预设不受影响；④**HexSpecs**（仿 CombatSpecs）：规格 post-pack + `sampleWheel`（真实 `FrozenPigment.getColor` 8 方向采样，任意第三方颜料兼容，最低亮度随采样带入）；⑤**HexSprayRedirectMixin**（client）：`MsgCastParticleS2C.Handler.handle` HEAD 注入——`hexSprayRedirect` 配置开（默认开）且引擎 available 时主线程采样→`spawnHexSpray`→cancel，否则原版路径直通（降级保底）；项目自有动作（OpLightBurner 等走 Hexcasting 喷洒管线）自动被同一 mixin 覆盖；⑥**/cmip spray <amethyst|uuid|rainbow> [count]**：StaffCastEnv 标志参数（上喷 1.5 b/s、fuzz 0.4、spread π/3）。已知偏差：引擎级远处淡出（原版无）；寿命连续化；色轮相位参数化与逐颜料 gradientDir 不逐项一致（视觉同构）。未移植：法术圆/常驻结阵渲染（非粒子系统）。**落地修复三连**：①`origin` 前向引用（style 4 分支位于 classic 段声明之前）→ 分支内自读命令字；②mixin 回调须 static（静态目标）；③**风暴门吞 style 4**（`> 2.5` 恒真）→ 收紧为 `>2.5 && <3.5`，否则 hex 命令被当风暴成员在零锚点生成；④**emitHex 脏标志**（仅 hex 排空写 true，槽位复用即污染后续命令的 c 槽——战斗粒子光照 vel.x≈0 全黑）→ 每帧构建前 `Arrays.fill` 复位；⑤**小跨度种子螺线**（hash1 的 `fract(p·0.1031)` 在 [0,1) 不折叠，同命令内粒子哈希随 inner 平滑漂移）→ 提取复用函数 **`wideSeed(base, inner)`**（`base·1e5 + inner·307`，~7.2 万次折叠，单条 FMA 表达式——跨端位级一致仅风暴身份需要，其余路径取性能最优），`seed` 在 main 顶部统一宽化：classic/combat/hex 三路全部生成哈希**与存储的 p3.z** 自动去相关；连带修正两个宽种子下游消费者——樱花 flutter 相位改 `cmiHash1(p3.z)·60°`（原裸 `p3.z·60°` 在 ~7e5 下 float sin/cos 精度崩坏）、死亡链 `s = p3.z + i·7.31` 去掉 `·91.7` 预放大（宽种子下 ULP≈8 会吞掉 7.31 步长导致 poof 结块；风暴 mseed 分布等价）
34. **战斗粒子对齐二轮（逐项核对 `CritParticle`/`TrackingEmitter`/`ExplodeParticle` 1.21.1 源码）** → ①**多人可见性**：暴击星/附魔星/红心原先只在攻击者本地生成（原版由服务器广播全员可见）——`ServerboundStormHitPacket` +1 字节 visuals（bit0 暴击、bit1 附魔、bit2-7 红心数≤31），`ClientboundStormDamagePacket` + 同字节 + 攻击者实体 id（VAR_INT，攻击者按 id 去重、其余客户端经 `CMIParticleEngine.spawnCombatVisuals` 生成，成员身份经 emit.comp memberKey 解析、缺席丢弃；攻击者保持点击瞬间本地生成 = 相对原版的已记录偏差，与其本地音效同风格）；/cmip 调试悦灵无服务器链路保持本地；②**poof 寿命形状**：原版 `16/(0.8r+0.2)+2` tick 偏短命分布替代均匀 0.9–4.1s（连续化、不整数化——引擎连续积分），死亡链直接计算，POOF 头寿命字段降级为纯范围文档；③**poof 尺寸形状**：原版 `0.1·(r1·r2·6+1)` 是两个独立均匀分布的乘积（均值 0.25、pdf −ln 形偏小）——旧审查报告误记为单变量平方（均值 0.3），死亡链改写 p0.w=(r1·r2·6+1)/3 精确复刻；④**暴击/红心基速度微抖动**：7 参 Particle 构造器的随机基速度经 CritParticle ×0.1 后（归一化立方体方向·(r+r+1)·0.04 b/tick + 0.01 up）此前被遗漏，emit.comp 新增 `critBaseJitter`（种子槽 61.1–65.1，与两个 style 现有槽位无冲突）补齐；⑤**暴击/红心尺寸区间**：emit style 的 p0.w 抖动 0.7..1.3 → [2/3, 4/3]，0.1125×区间 = 精确原版 [0.075, 0.15)；⑥**攻击音效**：非暴击分支按满蓄力分 `PLAYER_ATTACK_STRONG`/`PLAYER_ATTACK_WEAK`（原版门 `!crit && !sweep`，横扫分支不适用、疾跑击退后仍落 STRONG/WEAK；旧代码无 WEAK 分支）。横扫旋涡粒子（SWEEP_ATTACK）仍缺——单目标 GPU 实体无扫击 AoE 机制，维持有意省略
