 ...
 * fuel rod bloom shader (mirrors {@code MistClientHandler}).
 * <p>
...
public final class FuelRodBloomHandler {
    ...
    /** The glow cylinder's radius extends this far beyond the rod's diamond corner. */
    ...
    // Per-rod layout: x, y, z, radius, height, intensity (6 floats), max 32.
    private static final float[] rodData = new float[MAX_RODS * 6];
    ...
    static void onRodSync(FuelRodSync.RodSyncData data) {
        ...
        } else if (existing != null && existing.maxRadius == data.rod().maxRadius
                && existing.height == data.rod().height) {
    ...
    private static void packRodData() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            rodData[base + 3] = d.maxRadius + RADIUS_PADDING;
