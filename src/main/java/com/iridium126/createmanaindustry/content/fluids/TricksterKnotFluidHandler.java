...
public class TricksterKnotFluidHandler implements IFluidHandlerItem {
    ...
    public boolean isFluidValid(int tank, FluidStack stack) {
        return tank == 0 && stack.getFluid().isSame(CMIFluids.LIQUID_MANA.get());
    }

    @Override
    public int fill(FluidStack resource, IFluidHandler.FluidAction action) {
        if (resource.isEmpty() || !resource.getFluid().isSame(CMIFluids.LIQUID_MANA.get()))
            return 0;
    ...
    public FluidStack drain(int maxDrain, IFluidHandler.FluidAction action) {
        ...
        return drainedAmount > 0 ? new FluidStack(CMIFluids.LIQUID_MANA.get(), drainedAmount) : FluidStack.EMPTY;
    }
